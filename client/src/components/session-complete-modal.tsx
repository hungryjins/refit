...
export default function SessionCompleteModal({
  ...
  const formatDuration = (seconds: number) => {
    ...
    return minutes > 0
      ...
      : `${remainingSeconds}${t("time.seconds")}`;
  };

  const accuracy =
    sessionStats.totalAttempts > 0
      ? Math.round(
          (sessionStats.correctUsages / sessionStats.totalAttempts) * 100
        )
      : 0;
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg text-center border border-blue-200 dark:border-blue-800">
              ...
              <div className="text-sm text-blue-600 dark:text-blue-400">
                {t("session.completed.expressions")}
              </div>
            </div>
            ...
            <div className="bg-gray-50 dark:bg-gray-900/20 p-4 rounded-lg text-center border border-gray-200 dark:border-gray-800">
              ...
              <div className="text-sm text-gray-600 dark:text-gray-400">
                {t("session.total.attempts")}
              </div>
            </div>
          </div>

          {/* Practiced expressions list */}
          <div>
              ...
              {sessionStats.expressionResults.map((result, index) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <Badge
                      variant={result.correctUsage ? "default" : "secondary"}
                    >
                    ...
                    <Badge
                      ...
                      className={
                        result.correctUsage ? "bg-green-600" : "bg-red-600"
                      }
                    >
                      {result.correctUsage
                        ? t("session.success")
